from lib.core import LEAF_PATH, issue, revoke, gen_crl
from os import path
import argparse
import shutil
...
def _copy_wrap(args):
    ...
    try:
        p12 = path.join(LEAF_PATH, '%s.p12' % args.cn)
...
def main():
    ...
    parser.add_argument('mode', choices=['issue', 'revoke', 'crl'],
                        help='set cert command')
    parser.add_argument('-t', dest='type', choices=['user', 'service'],
    ...
    if args.mode in ['issue', 'revoke'] and not args.cn:
        parser.error('issue and revoke commands require -c CN')
    elif args.mode == 'issue' and not args.type:
        parser.error('issue requires -t TYPE')

    p12 = path.join(LEAF_PATH, '%s.p12' % args.cn)
    if args.mode == 'issue' and path.isfile(p12):
        print('* There exist an cert for this CN. Cert was not issued.')
        print('* Run revoke command with -c CN will revoke the given cert.')
        _copy_wrap(args)
